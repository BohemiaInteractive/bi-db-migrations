...
function populateMigrationDefinitions(tableMigrations, latestTag, projectRoot) {
    return Promise.all([
        populateMigrationDefinitionsFromFS(tableMigrations, latestTag, projectRoot),
        populateMigrationDefinitionsFromGit(tableMigrations, latestTag, projectRoot),
 ...
 * @return {Object}
 */
function populateMigrationDefinitionsFromGit(tableMigrations, latestTag, projectRoot) {
 ...
 * @return {Object}
 */
function populateMigrationDefinitionsFromFS(tableMigrations, latestTag, projectRoot) {
    return Promise.map(tableMigrations, function(mig) {
        let seedPath = path.resolve(mig.path + path.sep + 'seed.sql')
        ,   schemaPath = path.resolve(mig.path + path.sep + 'schema.sql');
 ...
 * @return {Promise}
 */
function createPlainSqlMigration(tables, version, migDir, provider, type) {
    let seedData   = ''
    ,   ext        = '.' + type
