...
Migration.prototype._getSequelize = function(verbose) {
    ...
    if (!(this.sequelize instanceof Sequelize)) {
        ...
        if (opt.dialect === 'mysql') {
            dialectOpt.multipleStatements = true;
        }
...
Migration.prototype._init = Promise.method(function(argv) {
    ...
    return this._inspectDir(argv, this.ROOT, [
        argv['mig-dir'] + '/src'
 ...
 * @param {Array<String>} [dirs] - required FS directories relative to project root
 *
 * @return {Promise}
 */
Migration.prototype._inspectDir = function _inspectDir(argv, projectRoot, dirs) {
    this.MIG_DIR = this._getMigDir(argv, projectRoot);

    if (dirs instanceof Array) {
        dirs.unshift(argv['mig-dir']);
    } else {
        dirs = [argv['mig-dir']];
    }
    ...
    return utils.assertFSDirs(projectRoot, dirs);
};
